...
describe('database-query', () => {
  ...
  it('should get kitties with auction data with query and orderby', async () => {
    const database = new Database(config.database);
    await database.open();
    const res = await database.getKittiesWithAuctions(null, ['k.ID DESC'], 20);
    assert(res && res.rows && res.rows.length === 20, 'Invalid result');
  }).timeout(10000);
  ...
  it('should sort by price', async () => {
    const database = new Database(config.database);
    await database.open();
    const orderby = 'a.StartPrice';
    const res = await database.getAuctions('k.GenesEyeType & 255 = 6', [orderby], 20);
    assert(res && res.rows && res.rows.length === 20, 'Invalid result');
  }).timeout(10000);
});
